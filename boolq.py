...
class BoolQDataset(Dataset):
    ...
    def __init__(self, dataframe, tokenizer, max_seq_length=256):
        ...
        self.encoded_data = data_utils.encode_data(dataframe, tokenizer, max_seq_length) #self.encoded_data should be a tuple of 2 tensors
        
        ## TODO: Use extract_labels() from data_utils to store the labels.
        self.label_list = data_utils.extract_labels(dataframe) #should be a list of labels (0,1..) 
    ...
    def __getitem__(self, i):
        ...
        input_ids = self.encoded_data[0] 
        attention_mask = self.encoded_data[1]
        labels = torch.Tensor(self.label_list)
        
        output = {'input_ids': input_ids[i], 'attention_mask': attention_mask[i], 'labels': labels[i]}
        return output 
