...
class TestBoolQDataset(unittest.TestCase):
    ...
    def test_item(self):
        ...
        for i in range(len(self.boolq_dataset)):
            output_dict = self.boolq_dataset[i]

            #get keys
            keys = list(output_dict.keys())
            #compare keys
            self.assertEqual(keys, ['input_ids', 'attention_mask', 'labels'])
            #get length
            input_ids_len = len(output_dict['input_ids'])
            attention_mask_len = len(output_dict['attention_mask'])
            #compare length
            self.assertEqual(input_ids_len, self.max_seq_len)
            self.assertEqual(attention_mask_len, self.max_seq_len)
            #compare type
