...
trainer = transformers.Trainer(model_init = finetuning_utils.model_init, args = training_arg,compute_metrics = finetuning_utils.compute_metrics, train_dataset = train_data)
...
bestrun = trainer.hyperparameter_search(hp_space = lambda _ : {"learning rate": tune.uniform(1e-5, 5e-5)}, n_trials = 3, search_alg = BayesOptSearch(), metric = 'eval_loss', mode = 'min')
